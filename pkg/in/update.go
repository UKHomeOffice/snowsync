...
func (p *Processor) setStatus(inc *Incident) (string, error) {
	...
	//FIXME: temp URL
	path, err := url.Parse("/tabf69k/rest/api/2/issue/" + inc.ExtID + "/transitions")
	if err != nil {
		return "", fmt.Errorf("could not form JSD URL: %v", err)
	}
...
func setPriority(inc Incident) error {
	...
	case "ServiceNow updated Priority to 5":
		inc.Priority = "P4 - General request"
	default:
		return fmt.Errorf("unexpected priority: %v", inc.Priority)
	}
